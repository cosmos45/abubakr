...
class CategoryPage {
    ...
    async init() {
        try {
            ...
            this.products = productData.getProductsByCategory(parseInt(this.categoryId));
        ...
        const headerHtml = `
            <div class="category-header">
                <div class="breadcrumb">
                    <a href="/abu-bakr-store-v2/">Home</a> /
                    ${this.category.parentId ? 
                        `<a href="/abu-bakr-store-v2/pages/category/category-page.html?id=${this.category.parentId}">
    ...
    applyFilters() {
        ...
        const productsToRender = activeFilters.size === 0
            ? [...this.originalProducts] // Use a copy of the original products
            : this.originalProducts.filter(product => {
                return Array.from(activeFilters.entries()).every(([filterType, filterValues]) => {
                    const productChoices = product.choices?.[filterType];
                    return productChoices && productChoices.some(choice => filterValues.has(choice));
                });
            });
    ...
    async renderFilteredProducts(products) {
        ...
        if (products.length === 0) {
            productsGrid.innerHTML = '<div class="no-products">No products match the selected filters</div>';
            return;
        }

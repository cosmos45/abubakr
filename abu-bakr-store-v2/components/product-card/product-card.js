...
export class ProductCard {
    ...
    static async loadTemplate() {
        try {
            const response = await fetch('/abu-bakr-store-v2/components/product-card/product-card.html');
    ...
    async render() {
        ...
        return renderedTemplate
            .replace('${oldPrice ? `<span class="old-price">£${oldPrice}</span>` : \'\'}', oldPriceHtml)
    ...
    static initializeCardListeners(cardElement) {
        ...
        minusBtn?.addEventListener('click', () => this.handleQuantityChange(input, false));
        plusBtn?.addEventListener('click', () => this.handleQuantityChange(input, true));
